import React from 'react'
import {post_body,post_image,post_desc,post_img,post_vid} from './styles/postcard.module.css'
import AccessTimeIcon from "@mui/icons-material/AccessTime";
import {GatsbyImage,getImage} from "gatsby-plugin-image"
import {Link} from "gatsby"
const Postcard=({data})=>{ 
 // console.log("All data",data)
const date = data?.postedAt;
const desc =  data?.excerpt;
const image = getImage(data?.media.gatsbyImage)
    return (
      <article className={post_body}>
        <div className={post_image}>
        {data?.type==="image article"?
          <GatsbyImage image={image} alt="desc" className={post_img}/>:
          <video src={data?.media.url} className={post_vid}/>}
        </div>
        {/* ---------------------------- */}
        <div className={post_desc}>
          <h3><Link to={`/blogs/${data?.slug}`}>{desc}</Link></h3>
          <p>
            <AccessTimeIcon style={{ fontSize: "14px" }} />
            &nbsp; {date}
          </p>
        </div>
      </article>
    );
}

export default Postcard